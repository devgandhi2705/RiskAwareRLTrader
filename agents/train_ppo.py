...
class DebugRewardCallback(BaseCallback):
    """
    V4-5: Logs full reward component breakdown for first N_DEBUG_EPISODES.
    Uses env.get_episode_debug_log() which has all regime + metric info.
    Active for both PPO and Safe PPO when safe_reward=True.
    """
    N_DEBUG_EPISODES = 5   # increased from 3 so we see more windows
    ...
    def _on_step(self):
        if self.locals["dones"][0] and self._ep_count < self.N_DEBUG_EPISODES:
            # Get the underlying env (unwrap VecEnv -> Monitor -> PortfolioTradingEnv)
            try:
                env = self.training_env.envs[0].env  # Monitor wraps the env
...
def train_ppo(safe_reward=False):
    ...
    for i, (tr_start, tr_end, val_end) in enumerate(WALK_FORWARD_WINDOWS):
        print(f"\n  Window {i+1}/5 | train: {tr_start}->{tr_end}  val: {tr_end}->{val_end}")
